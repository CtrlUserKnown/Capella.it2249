 ...
 * 
 * Pseudocode:
 * 1. Initialize 2D arrays for courses and validChoices
 * 2. Display menu and get user choice
 * 3. Validate choice against business rules
 * 4. Store valid registration in validChoices array
 * 5. Display current registrations
 * 6. Repeat until user enters "N"
 */
public class U8A1_2DConsoleRegisterForCourse {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("You may register for up to 3 courses with a maximum of 9 credit hours total.");
        System.out.println();
        ...
        do {
            ...
            // Check if user has reached maximum registrations or credits
            if (!validChoices[2][0].equals("")) {
                System.out.println("\nYou have registered for the maximum of 3 courses.");
                System.out.println("Registration complete. Exiting program.");
                break;
            }

            // Check if user cannot register for any more courses due to credit limit
            boolean canRegisterMore = false;
            for (int i = 0; i < courses[0].length; i++) {
                int courseCredits = Integer.parseInt(courses[1][i]);
                if (totalCredit + courseCredits <= 9) {
                    // Check if not already registered
                    String checkChoice = Integer.toString(i + 1);
                    if (!checkChoice.equals(validChoices[0][0]) && 
                        !checkChoice.equals(validChoices[1][0]) && 
                        !checkChoice.equals(validChoices[2][0])) {
            ...
            if (!canRegisterMore && !validChoices[0][0].equals("")) {
                System.out.println("\nYou cannot register for any additional courses without exceeding the 9 credit hour limit.");
                System.out.println("Registration complete. Exiting program.");
                break;
            }
    ...
    public static int ValidateChoice(int choice, String[][] validChoices, int totalCredit, String[][] courses) {
        ...
        if (choice < 1 || choice > 7) {
            return -1;
        } else if (choiceAsString.equals(validChoices[0][0]) || choiceAsString.equals(validChoices[1][0]) || choiceAsString.equals(validChoices[2][0])) {
            return -2;
